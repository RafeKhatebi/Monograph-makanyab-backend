...
<div style="background:linear-gradient(135deg,#064e3b,#10B981);padding:40px 0;">
    <div class="container">
        ...
        <p style="color:rgba(255,255,255,.8);margin:0;font-size:15px;">Latest news, updates and stories from Makanyab.</p>
    </div>
</div>
            ...
            <div class="row">
                @foreach($posts as $post)
                    <div class="col-md-4 col-sm-6" style="margin-bottom:28px;">
                        <div class="post-card" style="background:#fff;border-radius:14px;overflow:hidden;border:1px solid #E5E7EB;height:100%;display:flex;flex-direction:column;transition:box-shadow .2s;">
                            ...
                            <div style="padding:20px;flex:1;display:flex;flex-direction:column;">
                                <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                                    <span style="background:#ECFDF5;color:#10B981;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;">
                                        {{ $post->category->name ?? 'News' }}
                                    </span>
                                    <span style="color:#9CA3AF;font-size:13px;">{{ $post->created_at->format('M d, Y') }}</span>
                                </div>
                                <h3 style="font-size:17px;font-weight:700;color:#111827;margin-bottom:10px;line-height:1.4;flex:1;">
...
@push('styles')
<style>.post-card:hover { box-shadow: 0 4px 20px rgba(0,0,0,.08); }</style>
@endpush
