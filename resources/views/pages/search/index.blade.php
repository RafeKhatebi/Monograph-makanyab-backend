        ...
        <form action="{{ route('search.index') }}" method="GET" id="search-form">
            <div class="row" style="align-items:center;">
                ...
                <div class="col-md-2 col-sm-6" style="padding:0 6px;margin-bottom:10px;">
                    <div style="position:relative;">
                        <i class="fa fa-map-marker" style="position:absolute;left:14px;top:50%;transform:translateY(-50%);color:#9CA3AF;"></i>
                ...
                <div class="col-md-2 col-sm-6" style="padding:0 6px;margin-bottom:10px;">
                    <select name="type" style="width:100%;height:48px;padding:0 14px;border:1px solid #D1D5DB;border-radius:10px;font-size:15px;background:#fff;outline:none;">
                ...
                <div class="col-md-3 col-sm-12" style="padding-left:6px;margin-bottom:10px;display:flex;gap:8px;">
                    <button type="submit" style="flex:1;height:48px;background:#10B981;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:15px;cursor:pointer;">
                        <i class="fa fa-search"></i> Search
                    </button>
                    @if(request()->hasAny(['search','city','type','place_category','service_category','status','price_level','verified','province','district']))
                        <a href="{{ route('search.index') }}" style="height:48px;padding:0 16px;display:flex;align-items:center;border:1px solid #D1D5DB;border-radius:10px;color:#6B7280;text-decoration:none;">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px;">
                ...
                @foreach($placeCategories as $cat)
                    <a href="{{ route('search.index', array_merge(request()->except('place_category','service_category'), ['place_category'=>$cat->slug,'type'=>'places'])) }}"
                        ...
                        @if($cat->icon_name)<i class="fa {{ $cat->icon_name }}" style="margin-right:4px;"></i>@endif
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
                ...
                <div style="background:#fff;border-radius:14px;padding:24px;border:1px solid #E5E7EB;">
                    ...
                    <form action="{{ route('search.index') }}" method="GET">
                        ...
                        <div style="margin-bottom:16px;">
                            <label style="font-size:13px;font-weight:600;color:#374151;margin-bottom:6px;display:block;">Province</label>
                        ...
                        <div style="margin-bottom:16px;">
                            <label style="font-size:13px;font-weight:600;color:#374151;margin-bottom:6px;display:block;">District</label>
                            <input type="text" name="district" value="{{ request('district') }}" placeholder="District"
                                style="width:100%;height:42px;padding:0 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:14px;outline:none;">
                        </div>
                        <div style="margin-bottom:16px;">
                            <label style="font-size:13px;font-weight:600;color:#374151;margin-bottom:6px;display:block;">Place Category</label>
                            <select name="place_category" style="width:100%;height:42px;padding:0 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:14px;background:#fff;outline:none;">
                                <option value="">All Categories</option>
                                @foreach($placeCategories as $cat)
                                    <option value="{{ $cat->slug }}" {{ request('place_category')==$cat->slug ? 'selected':'' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div style="margin-bottom:16px;">
                            <label style="font-size:13px;font-weight:600;color:#374151;margin-bottom:6px;display:block;">Service Category</label>
                            <select name="service_category" style="width:100%;height:42px;padding:0 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:14px;background:#fff;outline:none;">
                                <option value="">All Categories</option>
                                @foreach($serviceCategories as $cat)
                                    <option value="{{ $cat->slug }}" {{ request('service_category')==$cat->slug ? 'selected':'' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div style="margin-bottom:16px;">
                        ...
                        <a href="{{ route('search.index') }}" style="display:block;text-align:center;margin-top:10px;color:#6B7280;font-size:13px;text-decoration:none;">
                            Reset All
                        </a>
                    </form>
                </div>
            ...
            <div class="col-md-9">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
                    <div>
                        <h3 style="font-size:20px;font-weight:700;color:#111827;margin:0;">
                            ...
                            @if(request('city')) in <span style="color:#10B981;">{{ request('city') }}</span>@endif
                        </h3>
                    </div>
                </div>
                ...
                @if($totalResults > 0)
                    ...
                    @if($showPlaces && $places && $places->count())
                        <h4 style="font-size:16px;font-weight:700;color:#374151;margin-bottom:14px;padding-bottom:8px;border-bottom:2px solid #E5E7EB;">
                        ...
                        <div class="row" style="margin-bottom:30px;">
                            @foreach($places as $place)
                                <div class="col-sm-6 col-md-4" style="margin-bottom:20px;">
                                    <div class="search-card" style="background:#fff;border-radius:14px;overflow:hidden;border:1px solid #E5E7EB;height:100%;display:flex;flex-direction:column;">
                                        <div style="position:relative;overflow:hidden;height:180px;">
                                            <a href="{{ route('places.show', $place) }}">
                                                @if($place->media->first())
                                                    <img src="{{ asset('storage/'.$place->media->first()->file_path) }}" alt="{{ $place->name }}" style="width:100%;height:180px;object-fit:cover;transition:transform .3s;">
                                                @else
                                                    <img src="{{ asset('assets/img/demo/property-1.jpg') }}" alt="{{ $place->name }}" style="width:100%;height:180px;object-fit:cover;">
                                                @endif
                                            </a>
                                            @if($place->is_verified)
                                                <span style="position:absolute;top:10px;left:10px;background:rgba(16,185,129,.9);color:#fff;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">
                                                    <i class="fa fa-check-circle"></i> Verified
                                                </span>
                                            @endif
                                            @if($place->status==='open')
                                                <span style="position:absolute;top:10px;right:10px;background:rgba(16,185,129,.9);color:#fff;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">Open</span>
                                            @elseif($place->status==='closed')
                                                <span style="position:absolute;top:10px;right:10px;background:rgba(239,68,68,.85);color:#fff;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">Closed</span>
                                            @else
                                                <span style="position:absolute;top:10px;right:10px;background:rgba(245,158,11,.85);color:#fff;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;">Temp. Closed</span>
                                            @endif
                                        </div>
                                        <div style="padding:14px 16px;flex:1;display:flex;flex-direction:column;">
                                            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
                                                <h5 style="margin:0;font-size:15px;font-weight:700;color:#111827;line-height:1.3;">
                                                    <a href="{{ route('places.show', $place) }}" style="color:inherit;text-decoration:none;">{{ $place->name }}</a>
                                                </h5>
                                                @if($place->category)
                                                    <span style="background:#F0FDF4;color:#10B981;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:600;white-space:nowrap;margin-left:6px;">{{ $place->category->name }}</span>
                                                @endif
                                            </div>
                                            ...
                                            @if($place->tagline)
                                                <p style="margin:0 0 10px;font-size:13px;color:#6B7280;line-height:1.5;flex:1;">{{ Str::limit($place->tagline, 70) }}</p>
                                            @endif
                                            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:auto;padding-top:10px;border-top:1px solid #F3F4F6;">
                                                <span style="font-size:12px;color:#9CA3AF;">{{ ucfirst($place->price_level) }}</span>
                                                <a href="{{ route('places.show', $place) }}" style="background:#10B981;color:#fff;padding:5px 14px;border-radius:8px;font-size:13px;font-weight:600;text-decoration:none;">View</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @elseif($showPlaces && $placeCount===0 && request('type')!=='services')
                        <div style="background:#fff;border-radius:14px;padding:30px;text-align:center;margin-bottom:24px;border:1px solid #E5E7EB;">
                                    ...
                                    <div class="search-card" style="background:#fff;border-radius:14px;overflow:hidden;border:1px solid #E5E7EB;height:100%;display:flex;flex-direction:column;">
                                        <div style="position:relative;overflow:hidden;height:180px;">
                                            <a href="{{ route('services.show', $service) }}">
                                                @if($service->media->first())
                                                    <img src="{{ asset('storage/'.$service->media->first()->file_path) }}" alt="{{ $service->name }}" style="width:100%;height:180px;object-fit:cover;">
                                                @else
                                        ...
                                        <div style="padding:14px 16px;flex:1;display:flex;flex-direction:column;">
                                            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
                                                ...
                                                @if($service->category)
                                                    <span style="background:#EFF6FF;color:#3B82F6;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:600;white-space:nowrap;margin-left:6px;">{{ $service->category->name }}</span>
                                                @endif
                                            </div>
                                            <p style="margin:0 0 8px;font-size:13px;color:#6B7280;">
                                                <i class="fa fa-map-marker" style="color:#EF4444;margin-right:4px;"></i>{{ $service->city }}
                                            </p>
                                            @if($service->description)
                                                <p style="margin:0 0 10px;font-size:13px;color:#6B7280;line-height:1.5;flex:1;">{{ Str::limit($service->description, 70) }}</p>
                                            @endif
                                            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:auto;padding-top:10px;border-top:1px solid #F3F4F6;">
                    ...
                    @elseif($showServices && $serviceCount===0 && request('type')!=='places')
                        <div style="background:#fff;border-radius:14px;padding:30px;text-align:center;border:1px solid #E5E7EB;">
                            <p style="color:#6B7280;margin:0;">No services found for these filters.</p>
                        </div>
                    @endif

                @else
                    <div style="background:#fff;border-radius:14px;padding:70px 30px;text-align:center;border:1px solid #E5E7EB;">
                        <div style="font-size:56px;margin-bottom:16px;">🔍</div>
                        <h3 style="font-size:22px;font-weight:700;color:#111827;margin-bottom:12px;">No results found</h3>
                        ...
                        <a href="{{ route('search.index') }}" style="background:#10B981;color:#fff;padding:12px 28px;border-radius:10px;font-weight:700;text-decoration:none;font-size:15px;">
                            Reset Search
                        </a>
                    </div>
                @endif
...
@push('styles')
<style>
    .search-card { transition: box-shadow .2s; }
    .search-card:hover { box-shadow: 0 4px 20px rgba(0,0,0,.08); }
    .search-card:hover img { transform: scale(1.03); }
</style>
@endpush
