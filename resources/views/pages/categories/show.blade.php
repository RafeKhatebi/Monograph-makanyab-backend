        ...
        <div style="display:flex;align-items:center;gap:14px;">
            ...
            <div>
                <div style="font-size:13px;color:rgba(255,255,255,.7);margin-bottom:4px;">
                    <a href="{{ route('categories.index') }}" style="color:rgba(255,255,255,.7);text-decoration:none;">Categories</a>
                    @if($category->parent) › <a href="{{ route('categories.show', $category->parent->slug) }}" style="color:rgba(255,255,255,.7);text-decoration:none;">{{ $category->parent->name }}</a>@endif
                </div>
                <h1 style="font-size:28px;font-weight:800;color:#fff;margin:0 0 4px;">{{ $category->name }}</h1>
                <p style="color:rgba(255,255,255,.8);margin:0;font-size:14px;">{{ $category->places_count }} place{{ $category->places_count !== 1 ? 's' : '' }}</p>
            </div>
        </div>
    </div>
</div>
...
<div style="background:#F8FAFC;padding:30px 0 70px;">
    <div class="container">
        ...
        @if($subcategories->count())
            <div style="background:#fff;border-radius:14px;border:1px solid #E5E7EB;margin-bottom:24px;overflow:hidden;">
                <button onclick="document.getElementById('subcats').classList.toggle('hidden')"
                    ...
                    <span><i class="fa fa-th-large" style="color:#10B981;margin-right:8px;"></i> Sub-categories ({{ $subcategories->count() }})</span>
                ...
                <div id="subcats" style="padding:0 20px 16px;display:flex;flex-wrap:wrap;gap:10px;">
                    @foreach($subcategories as $sub)
                        <a href="{{ route('categories.show', $sub->slug) }}"
                            ...
                            {{ $sub->name }}
                            <span style="background:#D1FAE5;padding:1px 7px;border-radius:20px;font-size:11px;">{{ $sub->places_count }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
            <h3 style="font-size:18px;font-weight:700;color:#111827;margin:0;">{{ $places->total() }} Place{{ $places->total() !== 1 ? 's' : '' }}</h3>
            <a href="{{ route('places.index', ['category' => $category->slug]) }}" style="font-size:13px;color:#10B981;text-decoration:none;">View with filters →</a>
        </div>
