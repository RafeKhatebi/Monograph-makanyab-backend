    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-3" style="margin-bottom:24px;">
                    ...
                    <form action="{{ route('services.index') }}" method="GET">
                        ...
                        <div style="margin-bottom:20px;">
                            <label style="font-size:13px;font-weight:600;color:#374151;display:flex;align-items:center;gap:8px;cursor:pointer;">
                                <input type="checkbox" name="verified" value="1" {{ request('verified') ? 'checked' : '' }} style="width:16px;height:16px;accent-color:#3B82F6;">
                                Verified Only
                            </label>
                        </div>
                        <button type="submit" style="width:100%;height:44px;background:#3B82F6;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:14px;cursor:pointer;">
                            Apply Filters
                        </button>
                        @if(request()->anyFilled(['search','city','category','status','price_level','verified']))
                            <a href="{{ route('services.index') }}" style="display:block;text-align:center;margin-top:10px;color:#6B7280;font-size:13px;text-decoration:none;">Reset All</a>
                        @endif
                    </form>
                </div>
                ...
                @if($categories->count())
                    <div style="background:#fff;border-radius:14px;padding:20px;border:1px solid #E5E7EB;margin-top:16px;">
                        <h5 style="font-size:14px;font-weight:700;color:#111827;margin-bottom:14px;">Service Categories</h5>
                        @foreach($categories as $cat)
                            <a href="{{ route('service-categories.show', $cat->slug) }}"
                                ...
                                <span>{{ $cat->name }}</span>
                                <span style="background:#EFF6FF;color:#3B82F6;padding:2px 8px;border-radius:20px;font-size:11px;font-weight:600;">{{ $cat->services_count ?? 0 }}</span>
                            </a>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="col-md-9">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
                    <h3 style="font-size:18px;font-weight:700;color:#111827;margin:0;">
                        ...
                        @if(request('search')) for "<span style="color:#3B82F6;">{{ request('search') }}</span>"@endif
                    </h3>
                </div>
                ...
                <div class="row">
                                    ...
                                    <a href="{{ route('services.show', $service) }}">
                                        @if($service->media->first())
                                            <img src="{{ asset('storage/'.$service->media->first()->file_path) }}"
                                                alt="{{ $service->name }}" style="width:100%;height:180px;object-fit:cover;transition:transform .3s;">
                                        @else
                    ...
                    @empty
                        <div class="col-md-12 text-center" style="padding:80px 0;">
                            ...
                            <h3 style="font-size:22px;font-weight:700;color:#111827;margin-bottom:12px;">No services found</h3>
                            <p style="color:#6B7280;margin-bottom:24px;">Try adjusting your filters.</p>
                            <a href="{{ route('services.index') }}" style="background:#3B82F6;color:#fff;padding:12px 28px;border-radius:10px;font-weight:700;text-decoration:none;">Reset Filters</a>
                        </div>
                    @endforelse
                </div>
