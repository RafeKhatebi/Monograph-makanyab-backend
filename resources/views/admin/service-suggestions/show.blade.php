    ...
    <div class="panel">
        <div class="panel-heading"
            ...
            <div>
                ...
                <p class="text-muted" style="margin:0;">Submitted by
                        ...
                        @if ($serviceSuggestion->suggestion_status === 'pending')
                            <form action="{{ route('admin.service-suggestions.approve', $serviceSuggestion) }}"
                                ...
                                <div style="margin-bottom:12px;">
                                    <label style="font-weight:600;color:#374151;margin-bottom:6px;display:block;">Admin
                                        Note</label>
                                    <textarea name="admin_note" rows="4"
                                        style="width:100%;padding:12px;border:1px solid #D1D5DB;border-radius:10px;outline:none;">{{ old('admin_note') }}</textarea>
                                </div>
                                <button type="submit" class="btn btn-success" style="width:100%;">Approve and
                                    Publish</button>
                            </form>
                            ...
                            <form action="{{ route('admin.service-suggestions.reject', $serviceSuggestion) }}"
                                ...
                                @csrf
                                <div style="margin-bottom:12px;">
                                    <label style="font-weight:600;color:#374151;margin-bottom:6px;display:block;">Rejection
                                        Note</label>
                                    <textarea name="admin_note" rows="4"
                                        style="width:100%;padding:12px;border:1px solid #D1D5DB;border-radius:10px;outline:none;">{{ old('admin_note') }}</textarea>
                                </div>
