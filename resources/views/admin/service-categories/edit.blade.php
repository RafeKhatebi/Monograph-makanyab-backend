...
@section('content')
    <section class="card admin-category-form" aria-label="{{ __('admin.crud.edit', ['item' => __('admin.dashboard.service_categories')]) }}">
        <div class="card-header admin-card-header">
            <h2 class="admin-card-title">{{ __('admin.crud.edit', ['item' => __('admin.dashboard.service_categories')]) }}</h2>
        ...
        <div class="card-body">
            <form action="{{ route('admin.service-categories.update', $serviceCategory) }}" method="POST" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="admin-form-grid">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $serviceCategory->name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" id="slug" name="slug" value="{{ old('slug', $serviceCategory->slug) }}"
                    ...
                    <div class="admin-full-span">
                        ...
                        <select id="parent_id" name="parent_id"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected(old('parent_id', $serviceCategory->parent_id) == $category->id)>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="admin-full-span admin-category-options">
                        <label class="admin-check-row">
                            <input type="checkbox" name="has_menu" value="1" @checked(old('has_menu', $serviceCategory->has_menu))
                                class="form-check-input">
                            <span>{{ __('admin.crud.has_menu') }}</span>
                        </label>
                        <label class="admin-check-row">
                            <input type="checkbox" name="has_booking" value="1" @checked(old('has_booking', $serviceCategory->has_booking))
                                class="form-check-input">
                            <span>{{ __('admin.crud.has_booking') }}</span>
                        </label>
                        <label class="admin-check-row">
                            <input type="checkbox" name="has_delivery" value="1" @checked(old('has_delivery', $serviceCategory->has_delivery))
                                class="form-check-input">
                            <span>{{ __('admin.crud.has_delivery') }}</span>
                        </label>
                        <label class="admin-check-row">
                            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $serviceCategory->is_active))
                                class="form-check-input">
